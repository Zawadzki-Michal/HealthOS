-- HealthOS: Phase 1 schema (issue #1)
-- Raw tables are locked down to service_role/owner in 0003_rls_policies.sql.
-- RLS is enabled here with zero policies so there is never a window where
-- a table exists without RLS on (default-deny until policies are added).
